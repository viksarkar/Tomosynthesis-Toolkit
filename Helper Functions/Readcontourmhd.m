function [x_dim,y_dim,z_dim,x_start,y_start,z_start,x_pixdim,y_pixdim,z_pixdim,filename,contourxoffset,contouryoffset,contourzoffset] = Readcontourmhd(fname)
...
try 
    ...
    for i = 1:length(header)
         ...
         if findstr('ContourOrigin',header{i})
             %read in x_start,y_start,z_start
             [A,B,C,D] = strread(header{i},'%s%f%f%f','delimiter','=');
             contourxoffset = B;
             contouryoffset = C;
